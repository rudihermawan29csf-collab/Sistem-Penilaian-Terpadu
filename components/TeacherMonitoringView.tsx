  ...
  const classStats = availableClasses.map(className => {
      ...
      const subjectStatus = subjectsList.map(subject => {
          ...
          if (subject === 'Pendidikan Agama Islam') {
              // Usually handled by specific teacher or admin, hard to map dynamically if multiple PAI teachers exist without mapping.
              // We'll search in teachers list just in case
