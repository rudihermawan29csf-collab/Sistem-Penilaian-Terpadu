                ...
                <React.Fragment key={chap.key}>
                    {!showUpColumn && fieldsToShow.map((f: any) => {
                        ...
                        <td key={f} className={`p-1 border-r border-gray-50 text-center ${unlocked ? 'bg-blue-50/30' : ''}`}>
                            {unlocked ? (
                                <input type="number" value={val ?? ''} onChange={(e) => handleInputChange(e, chap.key, f)} className={getScoreInputClass(val, true)} placeholder="-" />
                            ) : <span className="text-xs text-gray-300 block py-1.5">{val ?? '-'}</span>}
                        </td>
                    )})}
